...
execute if score #block_terms_enabled vars matches 1 if score #overpowered_terms_enabled vars matches 1 run scoreboard players set #word_execution_function vars 1

# If score not 0, run -- else do punish_word_disabled
execute if score #word_execution_function vars matches 1.. run function wordsmith:detect/words/wither_skeleton_skull2
